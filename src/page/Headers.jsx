  ...
  return (
    <>
      <div className="h-screen bg-userProfile-color">
        ...
        <div className="absolute right-0">
          <a href="https://github.com/zneret03/gitprofile">
            <img
              className="absolute right-0 py-2 px-3"
              src="./images/github.svg"
              alt="img"
            />
          </a>
          <img src="./images/triangleShape.svg" alt="img" />
        </div>
        {/**user avatar*/}
        <div className="flex justify-center">
          <div className="mt-10">
            <img
              ...
              src={`https://avatars3.githubusercontent.com/u/${users.id}?v=4`}
              alt="user avatar"
            />
          </div>
        </div>
        ...
        <div className="flex justify-center mt-1">
          <h2>
            <a href={users.html_url} className="text-white text-2xl a-color">
              @{users.login}
            </a>
          </h2>
        </div>
        ...
        <div className="flex content-start flex-wrap justify-center sm:justify-center py-1  mt-3">
          ...
          {users.company ? (
            <GitInformation className={"info mr-3"}>
              <i>
                <FontAwesomeIcon className="mr-3" icon={faBriefcase} />
              </i>
              {users.company}
            </GitInformation>
          ) : null}
          ...
          <GitInformation className="info">
            ...
            Joined{" "}
            {new Date(users.created_at).toLocaleDateString("en-US", {
              month: "long",
              day: "numeric",
              year: "numeric",
            })}
        ...
        <div className="flex justify-center">
          ...
          <StatsCard
            modal={(e) => modalFollowersOpen(e)}
            number={users.followers}
            stats={"FOLLOWERS"}
          />
          <StatsCard
            modal={(e) => modalFollowingsOpen(e)}
            number={users.following}
            stats={"FOLLOWING"}
          />
        </div>
      </div>
      {modalFollowers === true && (
        <Followers
          followers={followers}
          modal={(e) => modalFollowersClose(e)}
        />
      )}
      {modalFollowing === true && (
        <Following
          following={following}
          modal={(e) => modalFollowingClose(e)}
        />
      )}
    </>
  )
