...
const Chart = ({ langData, repoData }) => {
  ...
  const initLangChart = React.useCallback(() => {
  ...
  //Most starred chart
  const initStarredChart = React.useCallback(() => {
  ...
  const initStartPerLanguage = React.useCallback(() => {
  ...
  }, [initLangChart, initStarredChart, initStartPerLanguage])

  const chartSize = 300
  const langChartError = !(langChartData && langChartData.length > 0)
  const starredChartError = !(starChartData && starChartData.length > 0)
  const starPerLanguageError = !(
    starPerLanguageData && starPerLanguageData.length > 0
  )
  ...
  return (
    <>
      <div className="flex flex-col sm:flex-col lg:flex-row md:flex-col lg:flex-wrap items-center justify-center bg-color ">
        <ChartCard>
          {langChartError ? (
            <h1 className="text-3xl mx-1 mb-5 text-gray-500">
              Nothing to see here!
            </h1>
          ) : (
